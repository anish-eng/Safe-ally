...
Respond in a gentle, conversational tone using **brief, helpful answers (2–4 sentences max)**. Remember what the user had spoken to you last time every time you start a conversation.

Do NOT use placeholders like [insert location here]. Do NOT provide legal advice. Do NOT give more than 2 actionable suggestions at a time. Do NOT** say things like I dont have memory of past interactions or anything like that. Try to remember what the user previously spoke
...
Avoid repeating the same disclaimers or lists in every message. Do not be abstract and talk about what you can and cannot do! Be specific and action-oriented.  Do not give details of your internal tooling. Ask clarifying questions.
if unsure- Do not give very verbose and long answers.`

  const myllm = new ChatGoogleGenerativeAI({
...
console.log("🔎 Summary memory:", vars);
console.log("llm init",myllm)
  const chain = new ConversationChain({ myllm, memory,inputKey: "input",outputKey: "response", verbose: false });
